...
public class JdbcResultSet implements ResultSet {
    ...
    /** Query cursor. */
    private final QueryCursor<List<?>> cursor;
     ...
     */
    JdbcResultSet(JdbcStatement stmt, List<String> tbls, List<String> cols, List<String> types,
        int fetchSize, QueryCursor<List<?>> cursor) {
        assert stmt != null;
        assert tbls != null;
        assert cols != null;
        assert types != null;
    ...
    JdbcResultSet(JdbcStatement stmt, List<String> tbls, List<String> cols,
        List<String> types, Collection<List<?>> fields) {
        assert stmt != null;
        assert tbls != null;
        assert cols != null;
        assert types != null;
    ...
    @Override public boolean next() throws SQLException {
        ...
        if (!it.hasNext() || (stmt.getMaxRows() > 0 && pos >= stmt.getMaxRows())) {
            curr = null;
            ...
            for (Object val : row)
                curr.add(sqlType(val) ? val : val.toString());
